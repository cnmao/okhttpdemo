 ...
 * stored bytes exceeds （29maoczsd.超过） the limit, the cache will remove entries（条目） in the background until the limit
 * is satisfied.（满意的） The limit is not strict（严格）: the cache may temporarily（暂时） exceed it while waiting for
 * files to be deleted. The limit does not include filesystem overhead or the cache journal（日志） so
 * space-sensitive（空间敏感） applications should set a conservative（保守） limit.
 *
 ...
 *
 * <ul>
 *     <li>When an entry（条目） is being <strong>created</strong> it is necessary to supply a full set of
 *         values; the empty value should be used as a placeholder（占位符） if necessary.
 *     <li>When an entry is being <strong>edited</strong>, it is not necessary to supply（供应） data for
 ...
 *
 * <p>Clients call {@link #get} to read a snapshot（快照） of an entry. The read will observe the value at
 * the time that {@link #get} was called. Updates and removals after the call do not impact（碰撞） ongoing
 * reads.
 *
 * <p>This class is tolerant（宽容） of some I/O errors. If files are missing from the filesystem, the
...
public final class DiskLruCache implements Closeable, Flushable {
  ...
  private static final String READ = "READ";
     ...
     * constant（不变） string "libcore.io.DiskLruCache", the disk cache's version,
     * the application's version, the value count, and a blank line.
     *
     * Each of the subsequent lines in the file is a record of the state of a
     * cache entry. Each line contains space-separated（空间分隔） values: a state, a key,
     * and optional state-specific values.
     *   o DIRTY （脏）lines track（跟踪） that an entry is actively being created or updated.
     *     Every successful DIRTY action should be followed by a CLEAN or REMOVE
     *     action. DIRTY lines without a matching CLEAN or REMOVE indicate（表明） that
     *     temporary（临时） files may need to be deleted.
     *   o CLEAN lines track a cache entry that has been successfully published
     *     and may be read. A publish line is followed by the lengths of each of
     *     its values.
     *   o READ lines track accesses（访问） for LRU（this 缓存）.
     *   o REMOVE lines track entries that have been deleted.
     *
     * The journal file is appended（附加的） to as cache operations occur（发生）. The journal may
     * occasionally（偶尔） be compacted（压实） by dropping redundant（冗） lines. A temporary file named
     * "journal.tmp" will be used during compaction（压实）; that file should be deleted if
...
DiskLruCache内部日志文件，对cache的每一次读写都对应一条日志记录，DiskLruCache通过分析日志分析和创建cache。日志文件格式如下：
  ...
  //31maoczsd.DiskCacheLru初始化时通过读取日志文件创建cache容器：lruEntries。同时通过日志过滤操作不成功的cache项。同时通过日志过滤操作不成功的cache项。相关逻辑在DiskLruCache.readJournalLine,DiskLruCache.processJournal
  ...
  private final Runnable cleanupRunnable = new Runnable() { // maoczsd33.当冗余日志过多时，通过调用cleanUpRunnable线程重建日志
    public void run() {
      synchronized (DiskLruCache.this) {
        ...
        try {
          if (journalRebuildRequired()) { 
            rebuildJournal(); // 30maoczsd 初始化完成后，为避免日志文件不断膨胀，对日志进行重建精简，具体逻辑在DiskLruCache.rebuildJournal
            redundantOpCount = 0;
          }
        } catch (IOException e) {
